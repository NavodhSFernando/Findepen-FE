...
interface WithdrawFundsModalProps {
  ...
  onClose: () => void;
  goal: Goal | null;
  onWithdrawFunds: (amount: number, note?: string) => Promise<boolean>;
}
...
const WithdrawFundsModal: React.FC<WithdrawFundsModalProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    if (numAmount > goal.CurrentAmount) {
      Alert.alert(
        "Amount Too High",
        `The amount exceeds the current goal amount of Rs. ${goal.CurrentAmount.toFixed(
          2
        )}`
      );
      return;
    ...
    setLoading(true);
    try {
      const success = await onWithdrawFunds(
        numAmount,
        note.trim() || undefined
      );
      if (success) {
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.warningContainer}>
              ...
              <Text style={styles.warningText}>
                Withdrawing funds will reduce your goal progress and return the
